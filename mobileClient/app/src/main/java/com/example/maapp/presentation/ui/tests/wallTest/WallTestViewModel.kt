...
class WallTestViewModel @Inject constructor(
    ...
    init {
        repository.getBorderlineDevices()
            .schedulersIoToMain()
            .toObservable()
    ...
    fun sendData(response: DeviceResponse, mode: Modes) {
        if (mode.name == "PROCESSOR") {
            repository.sendDevice(response)
                .schedulersIoToMain()
                .subscribe({
                    Timber.d("Успешно")
                }, {
            ...
            repository.sendWallTestMemResult(response)
                .schedulersIoToMain()
                .subscribe({
                    Timber.d("Успешно")
                }, {
    ...
    suspend fun startProgressBar (view: View, mode: Modes) {
        ...
        if (mode.name == "PROCESSOR"){
            view.WallPbDescrip.text = "Подождите, происходит выполнение теста.\nРежим тестирования: процессор."
        }
        else {
            view.WallPbDescrip.text = "Подождите, происходит выполнение теста.\nРежим тестирования: процессор + память."
        }
        while (progressBarStatus < 100) {
            ...
            if(view.WallPrgView != null || view.WallPrgBar != null) {
                view.WallPrgView.text = "$progressBarStatus из 100%"
                view.WallPrgBar.secondaryProgress = progressBarStatus
            }
        }
    }
    ...
    fun showResult(view: View,
        ...
        if (borderlineDev.size == 0){
            view.WallResTextView.text = "Тест завершен. Результат выполнения теста на Вашем устройстве - " +
                    testResult + " м. "
            return
        }
        if (mode.name == "PROCESSOR"){
        ...
        if (bestResult > testResult) {
            ...
            view.WallResTextView.text = view.WallResTextView.text.toString() + "Это в $fmtStr раз меньше, чем тот результат, " +
                    "который показало 1 в рейтинге устройство - $bestDevTitle. "

        }
        else {
            if (testResult > bestResult) {
                view.WallResTextView.text =
                    view.WallResTextView.text.toString() + "Ваше устройство показало лучший результат. Ваша позиция в рейтенге - 1. "
            }
            else {
                view.WallResTextView.text =
                    view.WallResTextView.text.toString() + "Ваше устройство показало результат аналогичный результату лучшего устройства" +
                            "в рейтинге. Ваша позиция в рейтенге - 1. "
            }
        }

        if (worstResult < testResult) {
            diffRes = testResult.toFloat() / worstResult
            if (diffRes % 1 < 0.0f){
                fmtStr = "%d".format(diffRes.toInt())
            }
            else{
            ...
            view.WallResTextView.text = view.WallResTextView.text.toString() + "А также это в $fmtStr раз больше, чем результат " +
                    "последнего устройства в рейтинге - $worstDevTitle. "
        }
        else {
            view.WallResTextView.text = view.WallResTextView.text.toString() + "Ваше устройство показало худший результат среди всех устройств."
        }
    }
    ...
    fun runTask(view: View,
        ...
        runningDrawChartJob = GlobalScope.launch(Dispatchers.Main, start = CoroutineStart.LAZY) {
            view.runWallTask.isEnabled = false
            chartIn()
            chart.stackChart(mode, borderlineDev, chart_view, enableButton, "wall")
            if (borderlineDev.size >= 4) {
                if (mode.name == "PROCESSOR"){
                    chart.populateGraphData(borderlineDev[0].result, testResult, borderlineDev[1].result, chart_view, "wall")
                }
                else {
                    chart.populateGraphData(borderlineDev[2].result, testResult, borderlineDev[3].result, chart_view, "wall")
                }
            }
            else {
                chart.populateGraphData(0, testResult, 0, chart_view, "wall")
            }
        }
